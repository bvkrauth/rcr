...
program define rcr , eclass byable(recall)
    ...
    * vector of lengths
    quietly mat2txt, matrix(`length') saving("`input_file'") replace
    * Vector of moments
    ...
    * Get some data from the earlier estimation
    matrix `V' = e(V)
    * NOBS is the number of observations used for the calculation
    local nobs = e(N)
    * number of clusters
...
* The following program will calculate the confidence interval for betax and
* displays it at the bottom panel of the estimation table along with the CI
* type
program Footer_rcr
    syntax [, Level(cilevel)]
    if (e(citype) == "Conservative") ci_conservative, level(`level')
    ...
    * this creates the line that displays the CI for betax in the footer
    di as text %12s `"betax"' " {c |}" _skip(3) as text _skip "(" e(citype) _skip "confidence interval)" ///
        as result %9.0g _col(58) `betaxCI_L' _col(70) %9.0g `betaxCI_H'
    di as text in smcl "{hline 13}{c BT}{hline 64}"
    * this will display the name of treatment variable
    di as text `"Treatment Variable:"' as result _skip(3) e(treatvar)
    local maxlen 57
...
program define ci_imbensmanski, rclass
    syntax , Level(cilevel)
    * The Imbens-Manski critical value lies somewhere between CV_MIN (the
    * critical value for a one-tailed test) and CV_MAX (the critical value
