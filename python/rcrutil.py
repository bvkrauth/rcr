...
def get_command_arguments(args):
    ...
    print("infile = {0}, outfile = {1}, logfile = {2}, detail_file = {3}".format(infile,outfile,logfile,detail_file))
    write_to_logfile("Log file {0} for RCR version 1.0\n".format(logfile),mode="w")
    if (len(args) > 5):
        msg = "WARNING: Unused arguments passed to program {0}".format(args[5:])
...
def read_data(infile):
    ...
    write_to_logfile("Run at {0}.\n".format(datetime.now().strftime("%H:%M on %m/%d/%y")))
    write_to_logfile("Reading data from input file {0}.\n".format(infile))
    try:
        line1 = pd.read_csv(infile,sep="\s+",skiprows = [1,2],header=None).values[0,]
        n_moments, n_lambda, external_big_number =  tuple(line1)
    except FileNotFoundError:
        write_to_logfile("FATAL ERROR: infile {0} not found.\n".format(infile))
        return sys.exit()
    except ValueError:
        write_to_logfile("FATAL ERROR: Incorrect format in line 1 of infile {0}.\n".format(infile))
        return sys.exit()
    except:
        write_to_logfile("FATAL ERROR: Unknown problem with line 1 of infile {0}.\n".format(infile))
        return sys.exit()
    else:
        write_to_logfile("Line 1: n_moments = {0}, n_lambda = {1}, external_big_number = {2}.\n".format(n_moments,n_lambda,external_big_number))
    try:
        moment_vector = pd.read_csv(infile,sep="\s+",skiprows = [0,2],header=None).values[0,].astype(np.float64)
    except ValueError:
        write_to_logfile("FATAL ERROR: Incorrect format in line 2 of infile {0}.\n".format(infile))
        return sys.exit()
    except:
        write_to_logfile("FATAL ERROR: Unknown problem with line 2 of infile {0}.\n".format(infile))
        return sys.exit()
    else:
        write_to_logfile("Line 2: moment_vector = a vector of length {0}.\n".format(len(moment_vector)))
    try:
        lambda_range = pd.read_csv(infile,sep="\s+",skiprows = [0,1],header=None).values[0,].astype(np.float64)
    except ValueError:
        write_to_logfile("FATAL ERROR: Incorrect format in line 3 of infile {0}.\n".format(infile))
        return sys.exit()
    except:
        write_to_logfile("FATAL ERROR: Unknown problem with line 3 of infile {0}.\n".format(infile))
        return sys.exit()
    else:
    ...
    #   1. It should be twice the length of lambda_range.  if not, just reset it.
    if len(lambda_range) != 2*n_lambda:
        warn("n_lambda reset from {0} to len(lambda_range)/2 = {1}.".format(n_lambda,int(len(lambda_range)/2)))
    ...
    #   1. It should be the same as the length of moment_vector.  if not, just reset it.
    if n_moments != len(moment_vector):
        warn("n_moments reset from {0} to len(moment_vector) = {1}.".format(n_moments,len(moment_vector)))
        n_moments = len(moment_vector)
    #   2. It must be at least 9 (i.e., there must be at least one explanatory variable)
    assert n_moments >= 9
    ...
    # If external_big_number is bigger than sys.float_info.max, then issue a warning but don't stop program.
