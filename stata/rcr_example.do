...
* This example uses the kindergarten group from the Project STAR data set used
* in the paper. The data set has already been subject to the fixed effects
* transformation.
...
* The results of the RCR command are stored in e() and so can be saved and used
* in tables
...
* Note that in this particular example, the bounds on betax are
* (-infinity,infinity).  Since Stata has no facility for manipulating infinity,
* infinity is reported as just a very large number.
rcr SAT Small_Class White_Asian Girl Free_Lunch White_Teacher Teacher_Experience Masters_Degree, lambda(0 .)

* By default, confidence intervals are estimated conservatively. The CITYPE
* option allows the choice of other methods. */
rcr SAT Small_Class White_Asian Girl Free_Lunch White_Teacher Teacher_Experience Masters_Degree, citype("Imbens-Manski")

* The VCEADJ allows for the user to adjust the covariance matrix estimates by a
* multiplicative factor.  This can be used to implement degrees-of-freedom
* corrections when the data has been transformed (e.g., normalization or
...
* For hypothesis testing of point-identified parameters (including betaxH and
* betaxL), the TESTNL and NLCOM postestimation commands can be used
testnl _b[betaxL] = 0
nlcom _b[betaxH] - _b[betaxL]

* There is also a postestimation command test_betax for testing hypotheses
* about the interval-identified parameter betax. You can test any point null
* hypothesis.  This line of code tests H0: betax = 1.25
test_betax = 1.25
* If you call test_betax without any arguments, the default is to test
* H0: betax = 0.
test_betax
