...
* FW = frequency weights, should lead to double the number of observations and
* smaller SE's
...
* PW = probability weights, should lead to same number of observations and
* same SE's
...
assert reldif( e(betaxCI_H)  , 6.488085264868134 ) <  `tol'
assert reldif( e(betaxCI_L)  , 3.259480713112181 ) <  `tol'
* IW = importance weights, should lead to double the number of observations and
* smaller SE's
...
assert reldif( r(betaxCI_L)  , 3.561021633566327 ) <  `tol'
* Specify NONSENSE (or any other unsupported type).  Should generate a warning
* message and a missing CI
...
preserve
* The default scheme (s2color) changes slightly after version 8, so we use a
* scheme that didn't change
set scheme s2mono
...
* Save plot (as a postscript file) and check against previous plot to make sure
* it hasn't changed. If previous plot doesn't exist, create one
